...
const runnerDir = process.env.E2E_RUNNER_DIR
  ? path.resolve(process.env.E2E_RUNNER_DIR)
  : process.cwd();
const workspaceRoot = path.resolve(fileURLToPath(new URL('../../', import.meta.url)));
const muiSrc = path.resolve(fileURLToPath(new URL('../../sub/mui/src', import.meta.url)));
const muiHooksSrc = path.resolve(
  fileURLToPath(new URL('../../sub/mui/src/hooks', import.meta.url)),
);
  ...
  resolve: {
    alias: [
      ...
      // Force React resolution to come from the runner package (React 18 vs 19).
      {
        ...
        replacement: path.resolve(runnerDir, 'node_modules/react-dom/client'),
      },
